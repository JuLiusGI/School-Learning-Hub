            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('sections.update', $section) }}" class="space-y-6">
                        @csrf
                        @method('PUT')
                        <input type="hidden" name="updated_at" value="{{ $section->updated_at?->toDateTimeString() }}">
                        ...
                        <div>
                            ...
                            <select id="grade_id" name="grade_id" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                                ...
                                @foreach ($grades as $grade)
                                    <option value="{{ $grade->id }}" @selected(old('grade_id', $section->grade_id) == $grade->id)>{{ $grade->level }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select id="adviser_user_id" name="adviser_user_id" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                                ...
                                @foreach ($advisers as $adviser)
                                    <option value="{{ $adviser->id }}" @selected(old('adviser_user_id', $section->adviser_user_id) == $adviser->id)>{{ $adviser->name }}</option>
                                @endforeach
                            </select>
