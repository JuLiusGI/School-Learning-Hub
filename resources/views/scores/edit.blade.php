        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('scores.update', $score) }}" class="space-y-6">
                        @csrf
                        @method('PUT')
                        <input type="hidden" name="updated_at" value="{{ $score->updated_at?->toDateTimeString() }}">
                        ...
                        <div>
                            ...
                            <select id="assessment_id" name="assessment_id" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                                ...
                                @foreach ($assessments as $assessment)
                                    <option value="{{ $assessment->id }}" @selected(old('assessment_id', $score->assessment_id) == $assessment->id)>
                                        {{ $assessment->title }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            <div>
                                ...
                                <x-text-input id="score" name="score" type="number" min="0" class="mt-1 block w-full" value="{{ old('score', $score->score) }}" required />
                                <x-input-error class="mt-2" :messages="$errors->get('score')" />
                            </div>
                            <div>
                                <x-input-label for="remarks" value="Remarks (Optional)" />
                                <x-text-input id="remarks" name="remarks" type="text" class="mt-1 block w-full" value="{{ old('remarks', $score->remarks) }}" />
