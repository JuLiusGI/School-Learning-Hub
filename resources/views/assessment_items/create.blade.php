        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('assessment-items.store') }}" class="space-y-6">
                        @csrf
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            <div>
                                ...
                                <x-text-input id="item_no" name="item_no" type="number" min="1" class="mt-1 block w-full" value="{{ old('item_no') }}" required />
                                <x-input-error class="mt-2" :messages="$errors->get('item_no')" />
                            </div>
                            <div>
                                <x-input-label for="points" value="Points" />
                                <x-text-input id="points" name="points" type="number" min="0" class="mt-1 block w-full" value="{{ old('points') }}" required />
                        ...
                        <div>
                            ...
                            <textarea id="question_text" name="question_text" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" rows="4" required>{{ old('question_text') }}</textarea>
                            <x-input-error class="mt-2" :messages="$errors->get('question_text')" />
                        </div>

                        <div>
                            <x-input-label for="correct_answer" value="Correct Answer (Optional)" />
                            <textarea id="correct_answer" name="correct_answer" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" rows="3">{{ old('correct_answer') }}</textarea>
